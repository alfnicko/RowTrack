...
// Get all athletes in coach's team
$athletes_query = "SELECT u.id, u.username 
                   FROM users u
                   JOIN teams t ON u.team_id = t.id
                   WHERE t.coach_id = ? AND u.role = 'athlete'
                   ORDER BY u.username";
$athletes_stmt = $conn->prepare($athletes_query);
$athletes_stmt->bind_param("i", $coach_id);
$athletes_stmt->execute();
$athletes_result = $athletes_stmt->get_result();

$athletes = [];
while ($row = $athletes_result->fetch_assoc()) {
    $athletes[$row['id']] = $row['username'];
}
...
<body>
    ...
    <main class="container">
        ...
        <div class="card athlete-selector">
            <label for="athlete-select"><i class="fas fa-user"></i> Select Athlete:</label>
            <select id="athlete-select" onchange="window.location.href='?athlete_id='+this.value+'&type=<?= $selected_type ?>'">
                <?php foreach ($athletes as $id => $name): ?>
                    <option value="<?= $id ?>" <?= $selected_athlete_id == $id ? 'selected' : '' ?>>
        ...
        <?php else: ?>
            ...
            <div class="card">
                <h3>Recent Sessions for <?= htmlspecialchars($athletes[$selected_athlete_id]) ?></h3>
